...
- **Compatibility**: No breaking changes. Default behavior remains unchanged (flag is opt-in). Works on Unix-like filesystems and Windows NTFS; falls back to copy on FAT32/exFAT, cross-device paths, Docker separate mounts, or any other `os.Link` failure.
